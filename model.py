...
class Model:
    def __init__(self):
        ...
        self.model_name = "Faster R-CNN (DB-ResNet50)"

    def _download_and_extract(self, model_name_key: str, out_dir: pathlib.Path) -> str:
        model_url = self.model_dict[model_name_key]["model"]
    ...
    def _load_models(self) -> dict[str, nn.Module]:
        ...
        loaded_models = {}
        for key, item_dict in self.model_dict.items():
            ...
            checkpoint_file_path = self._download_and_extract(key, weight_dir)
    ...
    def set_model_name(self, name: str) -> None:
        ...
        elif name in self.model_dict: # Check if model is known but not loaded
             # Basic lazy loading: if model wasn't loaded initially but is known
            print(f"Lazy loading model: {name}")
            item_dict = self.model_dict[name]
            weight_dir = app_dir / "weights"
            config_file_path = app_dir / item_dict["config"]
            if not config_file_path.exists():
                config_file_path = app_dir / "CBNetV2" / item_dict["config"].replace("CBNetV2/", "", 1)
            
            checkpoint_file_path = self._download_and_extract(name, weight_dir)
